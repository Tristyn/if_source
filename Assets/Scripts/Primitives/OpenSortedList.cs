...
/*This SortedList is taken from .NET 4.8 System.Collections.Generic.SortedList with modifications.
* Sorted list behaviour remains.
* The underlying arrays are now public, so you can binary search the array directly.
* Modifications
* - Major changes are between BEGIN CHANGES and END CHANGES comments
* - Renamed to OpenSortedList, removed namespace
    ...
    IDictionary<TKey, TValue>, IDictionary, IReadOnlyDictionary<TKey, TValue>
{
    ...
    // all entries added to the sorted list.
    //
    public OpenSortedList(int capacity)
    {
        if (capacity < 0)
    ...
    // 
    public OpenSortedList(IComparer<TKey> comparer)
        : this()
    {
        if (comparer != null)
        {
    ...
    // 
    public OpenSortedList(int capacity, IComparer<TKey> comparer)
        : this(comparer)
    {
        Capacity = capacity;
    }
    ...
    // 
    public OpenSortedList(IDictionary<TKey, TValue> dictionary)
    ...
    // subsequently added to the sorted list.
    // 
    public OpenSortedList(IDictionary<TKey, TValue> dictionary, IComparer<TKey> comparer)
    ...
    public TValue this[TKey key]
    {
        ...
        set
        {
            if (((object)key) == null) throw new ArgumentNullException();
            int i = Array.BinarySearch(keys, 0, _size, key, comparer);
            if (i >= 0)
            {
                values[i] = value;
    ...
    /// <summary>
    /// Returns the index of the smallest element greater than or equal to key.
    /// Returns Length - 1 if the key is greater than all elements.
    /// </summary>
    public int IndexOfKeyGreaterThanOrEqualTo(TKey key)
    {
        if (key == null)
            throw new ArgumentNullException();
    ...
    /// <summary>
    /// Returns the index of the greatest element less than or equal to key.
    /// Returns 0 if the key is smaller than all elements.
    /// </summary>
    public int IndexOfKeyLessThanOrEqualTo(TKey key)
    {
        if (key == null)
            throw new ArgumentNullException();
        int index = Array.BinarySearch(keys, 0, _size, key, comparer);
    ...
    {
        private OpenSortedList<TKey, TValue> _sortedList;
        private TKey key;
        ...
        {
            if (version != _sortedList.version) throw new InvalidOperationException();
    ...
    [Serializable()]
#endif
    public struct SortedListKeyEnumerator : IEnumerator<TKey>, IEnumerator
    {
        private OpenSortedList<TKey, TValue> _sortedList;
        private int index;
        private int version;
        ...
        internal SortedListKeyEnumerator(OpenSortedList<TKey, TValue> sortedList)
        {
    ...
    [Serializable()]
#endif
    public struct SortedListValueEnumerator : IEnumerator<TValue>, IEnumerator
    {
        private OpenSortedList<TKey, TValue> _sortedList;
        private int index;
        private int version;
    ...
    [Serializable()]
#endif
    public struct KeyList : IList<TKey>, ICollection
    {
        private OpenSortedList<TKey, TValue> _dict;

        internal KeyList(OpenSortedList<TKey, TValue> dictionary)
        {
            _dict = dictionary;
        }
        ...
        public SortedListKeyEnumerator GetEnumerator()
        {
        ...
        public int IndexOf(TKey key)
        {
            ...
            int i = Array.BinarySearch(_dict.keys, 0,
    ...
    [Serializable()]
#endif
    public struct ValueList : IList<TValue>, ICollection
    {
        private OpenSortedList<TKey, TValue> _dict;
        ...
        public SortedListValueEnumerator GetEnumerator()
        {
